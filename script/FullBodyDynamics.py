...
if __name__ == "__main__":
    ...
    start = time.time()
    traj = robot.solveFowardDyamic(0, 5, robot.T_sb, robot.V_b, dt=0.001)
    ...
    plt.plot(t_, x_, label="python_x")
    plt.plot(t_, z_, label="python_z")
    plt.plot(df["t"], df["x"], label="cpp_x")
    plt.plot(df["t"], df["z"], label="cpp_z")
    plt.legend()
    plt.show()
